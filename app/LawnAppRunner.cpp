...
int RunLawnApp(platform::IFileSystem& fileSystem, platform::IClock& clock, platform::IWindow& window, platform::IInput& input)
{
    ...
    const unsigned long long startupBeginMs = static_cast<unsigned long long>(clock.NowMilliseconds());
    ...
    const unsigned long long startupDeltaMs = startupReadyMs >= startupBeginMs ? (startupReadyMs - startupBeginMs) : 0;
    ...
    const unsigned long long shutdownDoneMs = static_cast<unsigned long long>(clock.NowMilliseconds());
