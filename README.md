...
- ✅ Wired startup window/input seams through `IWindow` + `IInput` in the shared runner path.
...
- [ ] Replace direct WinAPI calls in migrated pathways with interface calls. (Partial: startup path routes resource probe via `IFileSystem`, timing via `IClock`, and window title/cursor via `IWindow`; entry wiring is no longer `WinMain`-only.)
...
- [ ] Windowing: move window creation/mode/cursor handling behind `IWindow`. (Partial: startup path now injects `IWindow` and routes title through adapter.)
